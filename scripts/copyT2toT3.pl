...
foreach $line (@log) {
  ...
  if ($line =~  m/(${thispart})/) {
      ...
      # print("$ok \n");
      if ($ok == 0) {
	  print("lcg-cp -b -S CMS_T3 -D srmv2 -t 4800 --verbose --vo cms ${thissource}/${filename} srm://srm-cms.cern.ch:8443/srm/managerv2\?SFN=/castor/cern.ch/user/c/covarell/${thisdest}/${filename} \n");
	  system("lcg-cp -b -S CMS_T3 -D srmv2 -t 4800 --verbose --vo cms ${thissource}/${filename} srm://srm-cms.cern.ch:8443/srm/managerv2\?SFN=/castor/cern.ch/user/c/covarell/${thisdest}/${filename}");
      }
  };
};
