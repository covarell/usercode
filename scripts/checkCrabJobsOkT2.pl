...
# Find missing successful jobs in a list of T2 files
...
if (!($deepcheck == 1 || $deepcheck == 2)) { 
  print("Argument n. 3 must be: 1 (check only missing files) or 2 (check also files with abnormal size) \n");
  exit;
} 
