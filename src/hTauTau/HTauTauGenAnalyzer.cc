...
void HTauTauGenAnalyzer::analyze( const Event& e, const EventSetup& )
{
   ...
   for ( GenEvent::particle_const_iterator p = Evt->particles_begin(); p != Evt->particles_end(); ++p ) {
	 ...
	 for ( GenVertex::particles_out_const_iterator ap = endvert->particles_out_const_begin(); ap != endvert->particles_out_const_end(); ++ap ) {
	   if (abs((*ap)->pdg_id()) == 15) {  // tau 
	     GenVertex* tauvert = (*ap)->end_vertex();
	     if (!tauvert) {
	       cout << "ATTENTION: tau not decayed!" << endl;
	       break;
	     } else {
	       GenVertex::particles_out_const_iterator bp = tauvert->particles_out_const_begin();
	       if (abs((*bp)->pdg_id()) == 15) {  // tau -> tau fake vertex
                                                  // generated by Tauola
		 hPtTau->Fill((*bp)->momentum().perp());
		 hTauStatus->Fill((*bp)->status());
		 hEtaTau->Fill((*bp)->momentum().pseudoRapidity());
		 GenVertex* tauvert2 = (*bp)->end_vertex();
	 ...
	 for ( GenVertex::particles_out_const_iterator ap = endvert->particles_out_const_begin(); ap != endvert->particles_out_const_end(); ++ap ) {
	   if (abs((*ap)->pdg_id()) == 15) {
		 ...
		 GenVertex* tauvert2 = (*bp)->end_vertex();
		 for ( GenVertex::particles_out_const_iterator cp = tauvert2->particles_out_const_begin(); cp != tauvert2->particles_out_const_end(); ++cp ) {
		   TLorentzVector pta((*bp)->momentum().px(), (*bp)->momentum().py(),
				      (*bp)->momentum().pz(), (*bp)->momentum().e());
		   TLorentzVector pl((*cp)->momentum().px(), (*cp)->momentum().py(),
				     (*cp)->momentum().pz(), (*cp)->momentum().e());
