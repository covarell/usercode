...
void addDtoTree(char* inputFile, int LHCsqrts = 7, float minMzz = 100., float maxMzz = 1000., bool containsPt = false, bool containsY = false){
  ...
  if(!containsPt && !containsY)
    ...
    {
      sprintf(outputFileName,"../datafiles/%s_%d-%d_7TeV_withDiscriminantsPtY.root",inputFile,int(minMzz),int(maxMzz));
    }

  sprintf(xrootdFileName,"root://lxcms02//data/Higgs/rootuplesOut/310812/PRODFSR/%s.root",inputFile,int(minMzz),int(maxMzz));
  TFile* sigFile = TFile::Open(xrootdFileName);
