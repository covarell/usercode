...
RooDataHist* withSmartBinning(TH1F* source, float mass, RooRealVar* var, float min, float max, float minWeight, float sizeSig, int type) {
  ...
  for (Int_t i=1; i<=source->GetNbinsX(); i++) {
    ...
    float thispt_m = (source->GetXaxis()->GetBinCenter(i))/mass;
    if (thispt_m > lowlimit && thispt_m < highlimits.back()) {
      bool lastbin = false;
      if ((source->GetXaxis()->GetBinCenter(i+1))/mass >= highlimits.back()) lastbin = true;
...
void Run2D(RooWorkspace *ws, int mZZcenter) {
  ...
  sprintf(fileToSave,"figs/fitMass_%dGeV.png",int(mZZcenter));
  ...
  for (unsigned int iToy = 0; iToy < nToys; iToy++) {
    ...
    RooDataSet *dataToy = ws->pdf("allGG")->generate(RooArgSet(*(ws->var("ZZMass")),*(ws->var("pts_m"))),howManyEventsGG,Extended());
    RooDataSet *dataToy2 = ws->pdf("allVBF")->generate(RooArgSet(*(ws->var("ZZMass")),*(ws->var("pts_m"))),howManyEventsVBF,Extended());
    RooDataSet *dataToy3 = ws->pdf("allZZ")->generate(RooArgSet(*(ws->var("ZZMass")),*(ws->var("pts_m"))),howManyEventsBKG,Extended());
    ...
    if (iToy == 0) { // JUST DRAW THE FIRST EXPERIMENT
      ...
      RooBinning myRb(ws->var("pts_m")->getMin(), ws->var("pts_m")->getMax());
      myRb.addBoundary(2.);
      myRb.addBoundary(5.);
      ...
      RooPlot *tframe = ws->var("pts_m")->frame();      
      ...
      RooPlot* tframeres =  ws->var("pts_m")->frame(Title("Residuals Distribution")) ;
      tframeres->addPlotable(hresid,"P") ;  
	
      can3.cd(2);gPad->SetLogx();gPad->SetLogy();tframe->Draw();
      can3.cd(4);gPad->SetLogx();tframeres->Draw();

      sprintf(fileToSave,"figs/fit2D_%dGeV.png",int(mZZcenter));
  ...
  can4.cd(4);nll->Draw();
  sprintf(fileToSave,"figs/toyResults_%dGeV.png",int(mZZcenter));
  can4.SaveAs(fileToSave);

  return; 
}

void fitPToverM(float mZZcenter = 126., float mZZspread = 5., 
	   int LHCsqrts = 7, 
	   ...
	   bool run2D = false)
{
  ...
  RooRealVar* pt_m = new RooRealVar("pt_m","p_{T}^{H}/M_{4l}",varMinBkg,varMaxBkg,"");
  RooRealVar* pts_m = new RooRealVar("pts_m","p_{T}^{H}/M_{4l}",varMinSig,varMaxSig,"");
  ...
  RooDataHist* bkg = withSmartBinning(bkgH,mZZcenter,pt_m,varMinBkg,varMaxBkg,minWeight,sizeSig,0);
  bkg->SetName("bkg");

  for (Int_t i=0; i<bkg->numEntries(); i++) {

    const RooArgSet* aRow = bkg->get(i);
    RooRealVar* pt_mprime = (RooRealVar*)aRow->find("pt_m");
    pt_m->setVal(pt_mprime->getVal());
  ...
  if (isVBFsignal) {
    sigVBF = withSmartBinning(sigVBFH,mZZcenter, pts_m,varMinSig,varMaxSig,minWeight,sizeSig,3);
  ...
  RooDataHist* sig = withSmartBinning(sigH,mZZcenter, pts_m, varMinSig,varMaxSig,minWeight,sizeSig,1);
  sig->SetName("sig");
 
  for (Int_t i=0; i<sig->numEntries(); i++) {

    const RooArgSet* aRow = sig->get(i);
    RooRealVar* pt_mprime = (RooRealVar*)aRow->find("pts_m");
    pts_m->setVal(pt_mprime->getVal());
  ...
  /*if (LHCsqrts == 8) {
    m.setVal(0.38);   //m.setConstant(kTRUE);    
    n2.setVal(1.34);   //n2.setConstant(kTRUE);
  } else {
    m.setVal(0.38);   //m.setConstant(kTRUE);    
    n2.setVal(1.31);   //n2.setConstant(kTRUE);
  }
  n.setVal(1.00);   // n.setConstant(kTRUE);
  bb.setVal(5.44); // bb.setConstant(kTRUE);
  bb2.setVal(0.10); //bb2.setConstant(kTRUE);
  T.setVal(0.008);   // T.setConstant(kTRUE);
  fexp.setVal(0.0);    //fexp.setConstant(kTRUE);*/

  RooRealVar ms("ms","emme signal", 5.,0.1, 10.,"");
  RooRealVar ns("ns","enne signal", 0.93, 0.3, 15.);
  RooRealVar n2s("n2s","enne2 signal", 0.75, 0.3, 15.); 
  RooRealVar bbs("bbs","bibi signal",0.02, 0.0005, 0.3);
  RooRealVar Ts("Ts","tti signal",0.005,0.00000005,0.2);
  RooRealVar bb2s("bb2s","bibi2 signal",0.02, 0.0005, 0.1);
  RooRealVar fexps("fexps","f_exp signal",0.02, 0.0, 1.0);

  RooRealVar mv("mv","emme signal", 0.2,0.1, 2.,"GeV/c");
  ...
  RooTsallis3* rt = new RooTsallis3("rt","rt",*pts_m,mv,nv,n2v,bbv,bb2v,Tv,fexpv);
  ws->import(*rt);
  RooTsallis3* rt2 = new RooTsallis3("rt2","rt2",*pts_m,ms,ns,n2s,bbs,bb2s,Ts,fexps);
  ws->import(*rt2);
  ...
  RooPlot *framesig = pts_m->frame();
  sprintf(reducestr,"pts_m > %f && pts_m < %f",pts_m->getMin(),pts_m->getMax());
  ...
  int nbins2 = pts_m->getBinning().numBins();
  for (int i = 0; i < nbins2; i++) {
    ...
    if (!isVBFsignal) {
      thisweight = sig->weight(RooArgSet(*pts_m));
      sig->weightError(thiserrup, thiserrdown, RooAbsData::SumW2);
    } else {
      thisweight = sigVBF->weight(RooArgSet(*pts_m));
  ...
  int nbins = pt_m->getBinning().numBins();
  for (int i = 0; i < nbins; i++) {
 
    pt_m->setVal(xbkg[i]);
    float thisweight = bkg->weight(RooArgSet(*pt_m));
    double thiserrup, thiserrdown;
  ...
  RooPlot* ressig = pts_m->frame(Title("Residuals Distribution")) ;
  ...
  RooPlot* resbkg = pt_m->frame(Title("Residuals Distribution")) ;
  ...
  RooPlot* pulsig = pts_m->frame(Title("Pull Distribution")) ;
  ...
  RooPlot* pulbkg = pt_m->frame(Title("Pull Distribution")) ;
  ...
  if (isVBFsignal) sprintf(fileToSave,"figs/fitVbfBkg_%dGeV_%dTeV_all.png",int(mZZcenter),LHCsqrts);
  else sprintf(fileToSave,"figs/fitSigBkg_%dGeV_%dTeV_all.png",int(mZZcenter),LHCsqrts);
  can.SaveAs(fileToSave);
  
  RooTsallis3* rt4 = new RooTsallis3("rt4","rt4",*ws->var("pts_m"),*ws->var("m"),*ws->var("n"),*ws->var("n2"),*ws->var("bb"), 
  ...
  if (writeWeightHisto) {
    ...
    for (Int_t i=1; i<=ggH->GetNbinsX(); i++) {
      ws->var("pts_m")->setVal(ggH->GetXaxis()->GetBinCenter(i));
