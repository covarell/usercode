...
void effPlots(char* f1, string outname, string MCorRECO = "RECO", string monPath = ""){
  ...
  string paths[npaths]={"HLT_Ele10_LW_L1RDQMWenu","HLT_Ele10_LW_EleId_L1RDQMWenu"};
  string Histo_paths[npaths];
  for(int i=0;i<npaths;i++){
    Histo_paths[i]="DQMData/Run 1/HLT/Run summary/HLTEgammaValidation/"+paths[i]+"/";
  ...
  for(int path=0; path< npaths ; path++){
    ...
    // *** for total/gen ***
    name2 = hf1->GetXaxis()->GetBinLabel(hf1->GetNbinsX()-2);

    std::cout << name2 << std::endl;   
    
    string numname = Histo_paths[path] + name2 + "et_" + MCorRECO + "_matched" + monPath;
    string denomname;
    if (MCorRECO == "RECO") denomname = Histo_paths[path] + "reco_et" + monPath;
    else denomname = Histo_paths[path] + "gen_et";
    string numnameeta = Histo_paths[path] + name2 + "eta_" + MCorRECO + "_matched" + monPath;
    string denomnameeta;
    if (MCorRECO == "RECO") denomnameeta = Histo_paths[path] + "reco_eta" + monPath;
    ...
    string thename = paths[path] + name2 + "L1eteff";
    ...
    thename = paths[path] + name2 + "L1etaeff";
    ...
    for(int filter=1; filter < hf1->GetNbinsX()-2 ; filter++){
      ...
      string numname = Histo_paths[path] + name2 + "et_" + MCorRECO + "_matched" + monPath;
      string denomname = Histo_paths[path] + name1 + "et_" + MCorRECO + "_matched" + monPath;
      string numnameeta = Histo_paths[path] + name2 + "eta_" + MCorRECO + "_matched" + monPath;
      string denomnameeta = Histo_paths[path] + name1 + "eta_" + MCorRECO + "_matched" + monPath;
