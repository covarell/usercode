...
class Analysis : public IOCombined {

 public:

  // constructor
  //Analysis(IOCombined* ioc, Bool_t par);
  Analysis(IOCombined* ioc);
...
Analysis::Analysis(IOCombined* ioc) : 
  IOCombined(*ioc) //, 
  //  parmode(par)
{
...
void Analysis::analysis(TFile * rootFile, TCanvas* can,TString psname, TString selstr, TString floatVar, 
  Bool_t PxZoom, Int_t minhit,  Float_t* ms, Float_t* mr, Int_t* itp, Int_t nb, Int_t nitermost)
{
  ...
  for (Int_t ia=0;ia<NAli;ia++) {
   //    if (AliUse[ia]==1) {
    if (sel[ia]==1  && AliId[ia] != 369412868 ) {
    ...
    // && AliId[ia] != 369214980 && AliId[ia] != 369264132 && AliId[ia] != 369330436 && AliId[ia] != 369346564) {
     ...
     cout << AliId[ia] << " " << AliObjId[ia] << " " << c2m*(AbsPosAl[ia][itp[3]][0]- AbsPosOl[ia][0]) << " " << c2m*(AbsPosAl[ia][itp[3]][1]- AbsPosOl[ia][1]) << endl;
...
void Analysis::globalTitle(TString s)
{
  if (s!="") {
